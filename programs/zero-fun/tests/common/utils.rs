use anchor_lang::{error::Error as AnchorError, AccountSerialize, Space};
...
use zero_fun::{GameError, GameSession, GlobalState, ID as ZERO_FUN_PROGRAM_ID};
...
pub fn get_initializer_keypair() -> Keypair {
    Keypair::from_base58_string(
        "3JzA5QAwszDUeHVoK8jZwNNmCvKKDQjz6u47TuW1cVBvsCmvt9Fhpb1WvxyHi8xkrv66NGw8GSsKfiko7NnYbuCW",
    )
}
...
pub fn create_vault_account(svm: &mut LiteSVM, vault_pubkey: Pubkey, lamports: u64) {
    ...
    svm.set_account(vault_pubkey, account)
        .expect("Could not add in vault account");
}
...
pub fn assert_custom_transaction_error_at(
    ...
    let expected_error = match AnchorError::from(error) {
        ...
        AnchorError::ProgramError(program_error) => {
            panic!("Expected an Anchor error, but got a program error: {program_error:?}")
        }
    };
...
pub fn disable_signer(instruction: &mut Instruction, key: Pubkey) {
    let account_meta = instruction
        .accounts
        .iter_mut()
        .find(|account| account.pubkey.eq(&key));

    if account_meta.is_none() {
        panic!("Account not in instruction")
    }

    account_meta.map(|account_meta| account_meta.is_signer = false);
}
...
pub fn ed25519_instruction_for_parts(signer: &Keypair, parts: &[&[u8]]) -> Instruction {
